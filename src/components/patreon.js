const PatreonButton = ({ className, ...delegate }) => {
  return (
    <div
      className={`_2KV-widgets-shared--patreonWidgetWrapper patreonButton${
        className ? " " + className : ""
      }`}
      {...delegate}
    >
      <div className="fpvra8-0 Ubsnp">
        <a
          className="sc-AxmLO bVyoVa"
          color="primary"
          tabIndex="0"
          type="button"
          href="https://www.patreon.com/bePatron?u=28256207&redirect_uri=https%3A%2F%2Fbetteranimalsplus.com%2F&utm_medium=widget"
          role="button"
          target="_blank"
          rel="noopener noreferrer"
        >
          <div className="sc-AxgMl fMtbEW">
            <div className="sc-AxirZ dQIjTw">
              <span className="sc-AxjAm iKePQk">
                <svg viewBox="0 0 569 546" xmlns="http://www.w3.org/2000/svg">
                  <g>
                    <circle
                      cx="362.589996"
                      cy="204.589996"
                      data-fill="1"
                      id="Oval"
                      r="204.589996"
                    ></circle>
                    <rect
                      data-fill="2"
                      height="545.799988"
                      id="Rectangle"
                      width="100"
                      x="0"
                      y="0"
                    ></rect>
                  </g>
                </svg>
              </span>
            </div>
            <div className="sc-Axmtr kBWvho"></div>
            <span className="text">Become a patron</span>
          </div>
        </a>
      </div>
    </div>
  )
}
export default PatreonButton
